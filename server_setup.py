...
def build_docker_image(dockerfile_dir, image_name, password):
...
def create_certificate(host, password):
    """Create an SSL certificate for the given host."""
    print(f"Creating certificate for {host}...")
    certbot_command = f"certbot certonly --standalone --preferred-challenges http -d {host} --email sujith.niraikulathan@gmail.com --agree-tos --non-interactive"
    run_command_with_sudo(certbot_command,  password)
...
def setup_docker_user_group(env_file_path):
    ...
    with open(env_file_path, "a") as e:
        e.write(f"#Autogenerated file, if you wish to modify anything, re-run the script server_setup.py\n")
        docker_user = run_command("id -u $(whoami)")
        docker_group = run_command("id -g $(whoami)")
...
def create_nginx_configs_and_env(data, project_directory, password):
    ...
    for service, details in data.get('services', {}).items():
        ...
        if host and port:
            ...
            create_certificate(host, password)
...
def main():
    ...
    for repo_url, image_name in REPO_TO_IMAGE.items():
        ...
        build_docker_image(clone_dir, image_name, password)

    cloned_dir = clone_repo("git@github.com:sujithkanna/luffyhomes.git")

    run_command_with_sudo("systemctl stop nginx", password)
    ...
    run_command_with_sudo("systemctl start nginx", password)
